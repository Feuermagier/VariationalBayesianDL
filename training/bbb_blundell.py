...
def gaussian(x, mu, sigma):
    ...
    #return torch.log(torch.clamp(GAUSSIAN_SCALER / sigma * bell, 1e-10, 1.))  # clip to avoid numerical issues
    return torch.clamp(torch.distributions.Normal(mu, sigma, False).log_prob(x), -23, 0)

def scale_mixture_prior(input, PI, SIGMA_1, SIGMA_2):
    prob1 = math.log(PI) + gaussian(input, 0., SIGMA_1)
    prob2 = math.log(1. - PI) + gaussian(input, 0., SIGMA_2)
...
class BayesianLinear(nn.Module):
    ...
    def forward(self, input, infer=False):
        ...
        if self.hasScalarMixturePrior:  # for Scalar mixture vs Gaussian analysis
            ...
            self.lpw = gaussian(weight, 0, self.SIGMA_1).sum() + gaussian(bias, 0, self.SIGMA_1).sum()

        self.lqw = gaussian(weight, self.weight_mu, weight_sigma).sum() + gaussian(bias, self.bias_mu, bias_sigma).sum()
    ...
    def BBB_loss(self, input, target, variance, batch_idx = None):
        ...
        for _ in range(self.SAMPLES):
            ...
            if self.CLASSES > 1:
                ...
                sample_log_likelihood = -F.mse_loss(output, target, reduction="sum")
