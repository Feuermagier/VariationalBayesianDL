...
def bbb(layers, noise, learn_var, epochs, dataloader, batch_size, device, sampling="activations", mc_sample=5, prior=GaussianPrior(0, 1)):
    ...
    bbb_model = util.GaussWrapper(util.generate_model(layers, "relu", None, 
        linear_fn=lambda i, o: BBBLinear(i, o, prior, prior, device, mc_sample=mc_sample, sampling=sampling)), 
        noise, learn_var)
    bbb_model.to(device)
    ...
    for epoch in range(epochs):
        loss = run_bbb_epoch(bbb_model, optimizer, uncurried_nll_loss, dataloader, device)
        if epoch % 10 == 0:
